...
class _PropertyDetailsScreenState extends State<PropertyDetailsScreen> {
  ...
  final List<String> _plotTypes = ['Residential', 'Agricultural', 'Commercial', 'Industrial', 'Mixed Use'];
  final List<String> _facingList = ['East', 'West', 'North', 'South', 'North-East', 'North-West', 'South-East', 'South-West'];
  ...
  Widget _cardRow({required String label, String? sublabel, required Widget trailing, bool showBorder = true}) {
    return Container(
      ...
      decoration: BoxDecoration(
        border: showBorder ? const Border(bottom: BorderSide(color: AppColors.border, width: 0.8)) : null,
      ),
      child: Row(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (sublabel != null)
                  Text(sublabel, style: const TextStyle(fontSize: 12, color: AppColors.textLight)),
              ],
            ),
          ),
          trailing,
  ...
  Widget _editableRow(String label, TextEditingController ctrl, {bool showBorder = true, String prefix = '', String suffix = ''}) {
    return _cardRow(
      label: label,
      ...
      trailing: Row(
        ...
        children: [
          if (prefix.isNotEmpty)
            Text(prefix, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark)),
          SizedBox(
            width: 80,
            child: TextFormField(
              controller: ctrl,
    ...
    return GestureDetector(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(label, style: TextStyle(fontSize: 13, color: checked ? AppColors.textDark : AppColors.textMuted, fontWeight: checked ? FontWeight.w600 : FontWeight.w400)),
        ],
      ),
    );
  }
  ...
  Widget _yesNo(String label, String sublabel, bool value, Function(bool) onChanged) {
    return Padding(
              ...
              GestureDetector(
                ...
                child: Row(
                  children: [
                    ...
                    Text('Yes', style: TextStyle(fontSize: 13, color: value ? AppColors.primary : AppColors.textMuted, fontWeight: FontWeight.w600)),
                  ],
                ),
              ),
              ...
              GestureDetector(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 5),
                    Text('No', style: TextStyle(fontSize: 13, color: !value ? AppColors.primary : AppColors.textMuted, fontWeight: FontWeight.w600)),
                  ],
                ),
              ),
            ],
  ...
  void _showAddFacility(List<String> extraList) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('Add Facility', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700)),
        content: TextField(controller: _extraCtrl, autofocus: true,
            decoration: const InputDecoration(hintText: 'e.g. CCTV, Swimming Pool')),
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              if (_extraCtrl.text.trim().isNotEmpty) {
                setState(() { extraList.add(_extraCtrl.text.trim()); _extraCtrl.clear(); });
              }
    ...
    return Column(
      ...
      children: [
        ...
        _card([
          _cardRow(
            ...
            trailing: SizedBox(
              ...
              child: TextFormField(
                ...
                decoration: const InputDecoration(
                  hintText: '10', border: InputBorder.none, enabledBorder: InputBorder.none,
          ...
          _cardRow(
            ...
            trailing: DropdownButton<String>(
              value: _plotType,
              hint: const Text('Select', style: TextStyle(fontSize: 14, color: AppColors.textLight)),
              underline: const SizedBox(),
              icon: const Icon(Icons.keyboard_arrow_down_rounded, color: AppColors.textMuted, size: 20),
              style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark),
              items: _plotTypes.map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(),
              onChanged: (val) => setState(() => _plotType = val),
            ),
          ),
          _cardRow(
            label: 'Facing', showBorder: false,
            trailing: DropdownButton<String>(
              value: _facing,
              hint: const Text('Select', style: TextStyle(fontSize: 14, color: AppColors.textLight)),
              underline: const SizedBox(),
              icon: const Icon(Icons.keyboard_arrow_down_rounded, color: AppColors.textMuted, size: 20),
              style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark),
              items: _facingList.map((f) => DropdownMenuItem(value: f, child: Text(f))).toList(),
        ...
        _card([
          ...
          _cardRow(
            label: 'Plot Dimensions', showBorder: false,
            trailing: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                SizedBox(width: 40, child: TextFormField(controller: _plotDimLCtrl, keyboardType: TextInputType.number, textAlign: TextAlign.center,
                    style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700),
                    decoration: const InputDecoration(hintText: '30', border: InputBorder.none, enabledBorder: InputBorder.none, focusedBorder: InputBorder.none, filled: false, isDense: true, contentPadding: EdgeInsets.zero))),
                const Text(' x ', style: TextStyle(fontSize: 14, color: AppColors.textMuted, fontWeight: FontWeight.w600)),
                SizedBox(width: 40, child: TextFormField(controller: _plotDimWCtrl, keyboardType: TextInputType.number, textAlign: TextAlign.center,
                    style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700),
                    decoration: const InputDecoration(hintText: '50', border: InputBorder.none, enabledBorder: InputBorder.none, focusedBorder: InputBorder.none, filled: false, isDense: true, contentPadding: EdgeInsets.zero))),
                const Text(' ft', style: TextStyle(fontSize: 13, color: AppColors.textMuted)),
              ],
            ),
          ),
        ]),
        ...
        _card([
          ...
          _cardRow(
            label: 'Price Per sq ft', showBorder: false,
            trailing: Text(pricePerSqft, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.primary)),
          ),
        ]),
        Container(
        ...
        _photoButton('Upload Plot Photos', _photoCount, () => setState(() => _photoCount++)),
        if (_photoCount > 0) ...[
          const SizedBox(height: 10),
          Wrap(spacing: 8, runSpacing: 8,
              children: List.generate(_photoCount, (i) => Container(
                width: 64, height: 64,
                decoration: BoxDecoration(color: AppColors.primaryLight, borderRadius: BorderRadius.circular(10), border: Border.all(color: AppColors.primary)),
                child: const Icon(Icons.image_rounded, color: AppColors.primary, size: 28),
              ))),
        ],
        const SizedBox(height: 16),
        TextFormField(
          controller: _descriptionCtrl, minLines: 3, maxLines: 6,
          decoration: const InputDecoration(hintText: 'Enter more description about Plot', alignLabelWithHint: true),
        ),
        const SizedBox(height: 16),
      ],
    );
    ...
    return Column(
      ...
      children: [
        ...
        _card([
          ...
          _cardRow(
            label: 'Advance Deposit', sublabel: '(Reserve / Booking Fee)',
            showBorder: false,
            trailing: Row(mainAxisSize: MainAxisSize.min, children: [
              const Text('Rs ', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
              SizedBox(width: 70, child: TextFormField(controller: _singleDepositCtrl, keyboardType: TextInputType.number, textAlign: TextAlign.right,
                  style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700),
        ...
        _card([
          ...
          _cardRow(
            label: 'Advance Deposit', showBorder: false,
            trailing: Row(mainAxisSize: MainAxisSize.min, children: [
              const Text('Rs  ', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
              SizedBox(width: 70, child: TextFormField(controller: _doubleDepositCtrl, keyboardType: TextInputType.number, textAlign: TextAlign.right,
                  style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700),
                  decoration: const InputDecoration(border: InputBorder.none, enabledBorder: InputBorder.none, focusedBorder: InputBorder.none, filled: false, isDense: true, contentPadding: EdgeInsets.zero))),
            ]),
          ),
        ...
        _card([
          ...
          _cardRow(
            ...
            trailing: Row(mainAxisSize: MainAxisSize.min, children: [
              const Text('Rs ', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
              SizedBox(width: 70, child: TextFormField(controller: _familyDepositCtrl, keyboardType: TextInputType.number, textAlign: TextAlign.right,
                  style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700),
                  decoration: const InputDecoration(border: InputBorder.none, enabledBorder: InputBorder.none, focusedBorder: InputBorder.none, filled: false, isDense: true, contentPadding: EdgeInsets.zero))),
            ]),
          ),
        ...
        _photoButton('Upload Room Photos', _guestPhotoCount, () => setState(() => _guestPhotoCount++)),
        if (_guestPhotoCount > 0) ...[
          const SizedBox(height: 10),
          Wrap(spacing: 8, runSpacing: 8,
              children: List.generate(_guestPhotoCount, (i) => Container(
                width: 64, height: 64,
                decoration: BoxDecoration(color: AppColors.primaryLight, borderRadius: BorderRadius.circular(10), border: Border.all(color: AppColors.primary)),
                child: const Icon(Icons.image_rounded, color: AppColors.primary, size: 28),
              ))),
        ...
        TextFormField(
          controller: _guestDescCtrl, minLines: 3, maxLines: 6,
          decoration: const InputDecoration(hintText: 'Enter more description about house and rules', alignLabelWithHint: true),
        ),
        const SizedBox(height: 20),

        // Kitchen questions
        _yesNo('Kitchen is available ?', '(Common Kitchen)', _commonKitchen, (val) => _commonKitchen = val),
        _yesNo('Kitchen is available ?', '(Private Kitchens)', _privateKitchen, (val) => _privateKitchen = val),
  ...
  Widget _buildFacilityGrid(Map<String, bool> facilities, List<String> extras) {
    ...
    for (int i = 0; i < allItems.length; i += 2) {
      ...
      Widget left = _facilityCheck(label1, checked1, isExtra1 ? () {} : () => setState(() => facilities[label1] = !facilities[label1]!));
      ...
      if (i + 1 < allItems.length) {
        ...
        right = _facilityCheck(label2, checked2, isExtra2 ? () {} : () => setState(() => facilities[label2] = !facilities[label2]!));
      } else {
        // Last item alone — show Add More in second column
        right = GestureDetector(
          onTap: () => _showAddFacility(extras),
          child: const Text('Add More', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.primary, decoration: TextDecoration.underline)),
        );
      }
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            _CheckChip(label: 'Boys', selected: _availableFor.contains('boys'), onTap: () => setState(() => _availableFor.contains('boys') ? _availableFor.remove('boys') : _availableFor.add('boys'))),
            _CheckChip(label: 'Girls', selected: _availableFor.contains('girls'), onTap: () => setState(() => _availableFor.contains('girls') ? _availableFor.remove('girls') : _availableFor.add('girls'))),
            _CheckChip(label: 'Boys & Girls', selected: _availableFor.contains('boys_girls'), onTap: () => setState(() => _availableFor.contains('boys_girls') ? _availableFor.remove('boys_girls') : _availableFor.add('boys_girls'))),
            _CheckChip(label: 'Family', selected: _availableFor.contains('family'), onTap: () => setState(() => _availableFor.contains('family') ? _availableFor.remove('family') : _availableFor.add('family'))),
          ],
        ),
        const SizedBox(height: 20),
        Row(
          children: [
            ...
            Container(
              ...
              child: Text(_totalRoomsPgCtrl.text, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textDark)),
            ),
            const SizedBox(width: 6),
            Column(children: [
              GestureDetector(
                onTap: () => setState(() { int v = int.tryParse(_totalRoomsPgCtrl.text) ?? 12; _totalRoomsPgCtrl.text = (v + 1).toString(); }),
                child: const Icon(Icons.keyboard_arrow_up_rounded, color: AppColors.primary, size: 28),
              ),
              GestureDetector(
                onTap: () => setState(() { int v = int.tryParse(_totalRoomsPgCtrl.text) ?? 12; if (v > 1) _totalRoomsPgCtrl.text = (v - 1).toString(); }),
        ...
        ...[
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: _occupancyType == item['value'] ? AppColors.primaryLight : AppColors.surface,
              borderRadius: BorderRadius.circular(10),
              border: Border.all(color: _occupancyType == item['value'] ? AppColors.primary : AppColors.border, width: 1.5),
            ),
            child: Row(children: [
              Icon(_occupancyType == item['value'] ? Icons.radio_button_checked_rounded : Icons.radio_button_unchecked_rounded,
                  color: _occupancyType == item['value'] ? AppColors.primary : AppColors.textLight, size: 20),
              const SizedBox(width: 10),
              Text(item['label']!, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500,
                  color: _occupancyType == item['value'] ? AppColors.primary : AppColors.textDark)),
            ]),
          ),
        )),
        const SizedBox(height: 12),
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                Text(
                  widget.propertyType == 'plot' ? 'Enter Plot Details' : 'Enter Property Details',
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        const SizedBox(width: 6),
                        Text(typeLabel, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.primary)),
                      ]),
                    ),
                    const SizedBox(height: 20),
                    ...
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        hintText: widget.propertyType == 'plot' ? 'Enter Name of your Plot' : 'Name of your Property',
                        prefixIcon: const Icon(Icons.home_outlined, color: AppColors.textLight, size: 20),
                      ),
                    ),
                    const SizedBox(height: 16),
                    ...
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        hintText: widget.propertyType == 'plot' ? 'Enter Location of Plot' : 'Enter Location of Property',
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(
                        ...
                        prefixIcon: Icon(Icons.place_outlined, color: AppColors.textLight, size: 20),
                      ),
                    ),
                    const SizedBox(height: 8),
                    ...
                    Builder(builder: (_) {
                      if (widget.propertyType == 'plot') return _buildPlotSection();
                      if (widget.propertyType == 'guest') return _buildGuestSection();
...
class _CheckChip extends StatelessWidget {
  ...
  const _CheckChip({required this.label, required this.selected, required this.onTap});
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          if (selected) const Padding(padding: EdgeInsets.only(right: 4), child: Icon(Icons.check_rounded, size: 14, color: AppColors.primary)),
          Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: selected ? AppColors.primary : AppColors.textMuted)),
        ]),
      ),
    );
  }
